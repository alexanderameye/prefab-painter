...
namespace PrefabPainter {
    ...
    public class PrefabPainter : EditorWindow
    {
        ...
        public PainterPalette paletteField = null; 
            ...
            switch (toolBar)
            {
                case 0:
                    ...
                    if (BrushSettingsFold)
                    {
                        ...
                        paletteField = (PainterPalette)EditorGUILayout.ObjectField("Palette", paletteField, typeof(PainterPalette), false);
                        if ( GUILayout.Button("Import", GUILayout.Width(EditorGUIUtility.currentViewWidth / 7)) ) 
                        {

                            if (paletteField != null)
                            {
                                listSize = paletteField.palette.Count;
                                objects = paletteField.palette;
                                paletteField = null;

                                foreach (PaintObject brush in objects) brush.gameObjectEditor = null;
                            }
                    ...
                    if (GUILayout.Button("Export Palette")) 
                    {

                        PainterPalette asset = ScriptableObject.CreateInstance<PainterPalette>();

                        asset.palette = objects;
